...
// 	String qryStmt = "SELECT  *"
	...
	int total_price=0;
	while (rs.next()) {
		out.println("<TR>a1a<form action='doUpdateShoppingCart' target='_self' >");
// 		out.println("<TD>2" + "<button type='button' onclick=\"removeFromShoppingCart(" + rs.getString("pd.pd_id") + ")\">移除</button>");
		out.println("<TD>3<input name='ctm_id' type='hidden' value='" + ctm_id + "'/>" + rs.getString("ctm.ctm_account"));
		out.println("<TD>4<input name='pd_id' type='hidden' value='" + rs.getString("pd.pd_id") + "'/>" + rs.getString("pd.pd_name"));
		out.println("<TD>5" + rs.getString("sc.sc_price"));
		out.println("<TD>6<input name='sc_number' type='number' min=1  value=" + rs.getString("sc.sc_number") +" />"
				   +"<input type='submit' value='修改' />");
// 				   +"<input type='submit' value='修改' id = 'abc' />");
		int sub_total_price = rs.getInt("sc.sc_price")*rs.getInt("sc.sc_number"); 
		out.println("<TD>7" + sub_total_price );
		out.println("</form>8</TR>");
